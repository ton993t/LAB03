...
static const uint16_t ID_INPUT              = 0xFF01;
...
static const uint8_t char1_name[]  = "ID_Input";
...
static const esp_gatts_attr_db_t gatt_db[HRS_IDX_NB] =
{
    ...
    [IDX_CHAR_VAL_A] =
    {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&ID_INPUT, ESP_GATT_PERM_READ | ESP_GATT_PERM_WRITE | ESP_GATT_PERM_READ_ENC_MITM,
...
void ssd1306_init() {
	...
	if (espRc == ESP_OK) {
		...
		ESP_LOGE(EXAMPLE_TAG, "OLED configuration failed. code: 0x%.2X", espRc);
	}
	i2c_cmd_link_delete(cmd);
}

void ssd1306_display_text(const void *arg_text, uint8_t cur_page) {
	...
	espRc = i2c_master_cmd_begin(I2C_NUM_0, cmd, 10/portTICK_PERIOD_MS);
	if (espRc == ESP_OK) {
		ESP_LOGI(EXAMPLE_TAG, "OLED set up print successfully");
	} else {
		ESP_LOGE(EXAMPLE_TAG, "OLED set up print failed. code: 0x%.2X", espRc);
	}
	...
	for (uint8_t i = 0; i < text_len; i++) {
		if (text[i] == '\n') {
			...
			espRc = i2c_master_cmd_begin(I2C_NUM_0, cmd, 10/portTICK_PERIOD_MS);
			if (espRc == ESP_OK) {
				ESP_LOGI(EXAMPLE_TAG, "OLED print 'enter' successfully");
			} else {
				ESP_LOGE(EXAMPLE_TAG, "OLED print 'enter'  failed. code: 0x%.2X", espRc);
			}
			...
			espRc = i2c_master_cmd_begin(I2C_NUM_0, cmd, 10/portTICK_PERIOD_MS);			
			if (espRc == ESP_OK) {
				ESP_LOGI(EXAMPLE_TAG, "OLED print successfully");
			} else {
				ESP_LOGE(EXAMPLE_TAG, "OLED print failed. code: 0x%.2X", espRc);
			}
        ...
        case ESP_GATTS_WRITE_EVT:
            if (!param->write.is_prep){
                ...
                if(gatt_db_handle_table[IDX_CHAR_VAL_A] == param->write.handle && param->write.len == 4) {
                    char string[9] = {0}; //string[8] = '\0'
                    uint8_t write_data1[4] = {0x20, 0x52, 0x05, 0x50};
                    uint8_t write_data2[4] = {0x20, 0x52, 0x05, 0x03};
                    uint8_t write_data3[4] = {0x20, 0x52, 0x04, 0x38};
                    uint8_t write_data4[4] = {0x20, 0x52, 0x06, 0x22};
                    uint8_t write_data5[4] = {0x20, 0x52, 0x08, 0x14};
                    if(memcmp(write_data1, param->write.value, param->write.len) == 0) {
                        sprintf(string, "%02x%02x%02x%02x", param->write.value[0], param->write.value[1], param->write.value[2], param->write.value[3]);
                        ESP_LOGI(EXAMPLE_TAG, "(3)***** id 1 write success: %s ***** \n", string);
                        ssd1306_display_text(string, cur_page ++);
                        
                    }
                    if(memcmp(write_data2, param->write.value, param->write.len) == 0) {
                        sprintf(string, "%02x%02x%02x%02x", param->write.value[0], param->write.value[1], param->write.value[2], param->write.value[3]);
                        ESP_LOGI(EXAMPLE_TAG, "(3)***** id 2 write success: %s ***** \n", string);
                        ssd1306_display_text(string, cur_page ++);
                        
                    }
                    if(memcmp(write_data3, param->write.value, param->write.len) == 0) {
                        sprintf(string, "%02x%02x%02x%02x", param->write.value[0], param->write.value[1], param->write.value[2], param->write.value[3]);
                        ESP_LOGI(EXAMPLE_TAG, "(3)***** id 3 write success: %s ***** \n", string);
                        ssd1306_display_text(string, cur_page ++);
                        
                    }
                    if(memcmp(write_data4, param->write.value, param->write.len) == 0) {
                        sprintf(string, "%02x%02x%02x%02x", param->write.value[0], param->write.value[1], param->write.value[2], param->write.value[3]);
                        ESP_LOGI(EXAMPLE_TAG, "(3)***** id 4 write success: %s ***** \n", string);
                        ssd1306_display_text(string, cur_page ++);
                        
                    }
                    if(memcmp(write_data5, param->write.value, param->write.len) == 0) {
                        sprintf(string, "%02x%02x%02x%02x", param->write.value[0], param->write.value[1], param->write.value[2], param->write.value[3]);
                        ESP_LOGI(EXAMPLE_TAG, "(3)***** id 5 write success: %s ***** \n", string);
                        ssd1306_display_text(string, cur_page ++);
...
void app_main(void)
{
    ...
    ESP_LOGI(EXAMPLE_TAG, "PLEASE INPUT MEMBER ID");
    ssd1306_display_text((const void*)"Enter member ID:\n", cur_page++);
}